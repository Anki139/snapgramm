...
export const sendConnectionRequest = async (req, res) => {
  try {
    const { userId } = req.auth();
    const { id } = req.body;
    //check is the user has sent more than 20 req in last 24 hours
    ...
    if (connectionRequests.length > 20) {
      return res.json({
        success: false,
        message: "you already reached the maximum number of follow request",
      });
    }
    ...
    } else if (connection && connection.status === "accept") {
      return res.json({
        success: false,
...
// accept the connections request
