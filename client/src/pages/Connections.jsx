...
import {
  ...
  dummyPendingConnectionsData as pendiingConnections,
} from "../assets/assets";
...
export default function Connections() {
  ...
  const dataArray = [
    ...
    { label: "Pendings", value: pendiingConnections, icon: UserRoundPen },
...
<b>{items.value.length}</b>
<p className="text-slate-600">{items.label}</p>
</div>
    ))
  }
</div>
      ...
      <div key={user._id} className="w-full max-w-88 flex gap-5 p-6 bg-white shadow rounded-md ">
        ...
        <div className="flex-1">
          ...
          <p className=" text-slate-500">{user.username}</p>
           <p className=" text-sm text-gray-700">{user.bio.slice(0,30)}...</p>
            <div className="flex max-sm:flex-col gap-2 mt-4">
           { 
            <button onClick={()=>navigate(`/profile/${user._id}`)} className='w-full text-sm p-2 rounded bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-700 active:scale-95 transition text-white cursor-pointer '>
View Profile
            </button>
            }
            {
              currentTab=== 'Followings' && (
                <button className='w-full text-sm p-2 rounded bg-slate-100 hover:bg-slate-200 text-black active:scale-95 transition cursor-pointer'>Unfollow</button>
              )
            }
             {
              currentTab=== 'Pendings' && (
                <button className='w-full text-sm p-2 rounded bg-slate-100 hover:bg-slate-200 text-black active:scale-95 transition cursor-pointer'>Accept</button>
              )
            }
             {
              currentTab=== 'Connections' && (
                <button onClick={()=>navigate(`/messages/${user._id}`)} className='w-full text-sm p-2 rounded bg-slate-100 hover:bg-slate-200 text-black active:scale-95 transition cursor-pointer flex items-center justify-center gap-1'>
